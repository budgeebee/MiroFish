...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        def do_search(scope: str, limit: int):
            max_retries = 3
            delay = 2.0
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.debug(f"检索第 {attempt + 1} 次失败: {str(e)[:80]}, 重试中...")
                        time.sleep(delay)
                        delay *= 2
                    else:
                        logger.debug(f"检索在 {max_retries} 次尝试后仍失败: {e}")
            return None
        
        try:
            edge_result = do_search("edges", 30)
            node_result = do_search("nodes", 20)
